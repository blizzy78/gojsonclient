...
// If an HTTP request fails, it is retried according to the retry function, up to the maximum number
// of attempts.
...
func do[Req any, Res any](ctx context.Context, req *Request[Req, Res]) (*Response[Res], *http.Response, error) { //nolint:cyclop // it's not too bad
	ctx, cancel := context.WithTimeout(ctx, req.client.requestTimeout)
	defer cancel()

	jsonReqData := bytes.Buffer{}

	if err := req.marshalRequestFunc(&jsonReqData, req.req); err != nil {
	...
	req.client.logger.Info("execute HTTP request",
		slog.String("uri", req.client.baseURI+req.uri),
	...
	default:
		if err = req.unmarshalResponseFunc(httpRes, &jsonRes); err != nil {
			return nil, httpRes, fmt.Errorf("decode response: %w", err)
		}
	}
